...
class EmployeeForm extends CFormModel
{
	...
	public function rules()
	{
		return array(
            ...
            array('id, code, name, company_id, contract_id, address, contact_address, phone, user_card, department, position, wage,time,
             start_time, end_time, test_type, test_start_time, sex, test_end_time, test_wage, word_status, city',
                'safe'),
			array('code','required'),
			array('name','required'),
	...
	public function retrieveData($index)
	{
		...
		if (count($rows) > 0)
		{
			foreach ($rows as $row)
			{
                ...
                $this->word_html = "<h3 class='text-center'>合同未找到，請重新生成</h3>";
                if(!empty($row["word_url"])){
                    $wordUrl = Yii::app()->basePath."/../".$row["word_url"];
                    if(file_exists($wordUrl)) {
                        $word = new MyWord();
                        $word->setDocx($wordUrl);
