...
class AppointTripSetForm extends CFormModel
{
	/* User Fields */
	public $id;
	public $employee_id;
	...
	public function setAttrForCopy($post){
        $row = Yii::app()->db->createCommand()->select("id,employee_id")->from("hr_appoint")
            ->where('id=:id',array(':id'=>$post["id"]))->queryRow();
        $this->detail=array();
        if($row){
            $this->employee_id = $row["employee_id"];
            $rows = Yii::app()->db->createCommand()->select("audit_user,z_index")->from("hr_appoint_info")
                ->where('appoint_id=:id',array(':id'=>$row["id"]))->queryAll();
            if($rows){
    ...
    public function validateName($attribute, $params){
        ...
        $row = Yii::app()->db->createCommand()->select("appoint_code")->from("hr_set_trip")
            ->where('employee_id=:employee_id and id!=:id',
                array(':employee_id'=>$this->employee_id,':id'=>$id))->queryRow();
        if($row){
            $message = "该员工的出差配置已被指定，指定编号：".$row["appoint_code"];
    ...
    public static function getAppointTripSet($employee_id){
        $list = false;
        $rows = Yii::app()->db->createCommand()->select("a.audit_user")
            ->from("hr_set_trip_info a")
            ->leftJoin("hr_set_trip b","b.id = a.appoint_id")
	...
	public function retrieveData($index)
	{
		...
		$sql = "select * from hr_set_trip where id=".$index." ";
		...
		if ($row!==false) {
            ...
            $sql = "select * from hr_set_trip_info where appoint_id=".$index." ";
    ...
    protected function saveDetail(&$connection)
    {
        ...
        foreach ($this->detail as $row) {
            ...
            switch ($this->scenario) {
                case 'delete':
                    $sql = "delete from hr_set_trip_info where appoint_id = :appoint_id";
                    break;
                case 'new':
                    if ($row['uflag']=='Y') {
                        $sql = "insert into hr_set_trip_info(
                ...
                case 'edit':
                    switch ($row['uflag']) {
                        case 'D':
                            $sql = "delete from hr_set_trip_info where id = :id";
                            break;
                        case 'Y':
                            $sql = ($row['id']==0)
                                ?
                                "insert into hr_set_trip_info(
									    appoint_id, audit_user, z_index,lcu
									) values (
									    :appoint_id,:audit_user,:z_index,:lcu
									)"
                                :
                                "update hr_set_trip_info set
...
//                print_r($sql);exit();
	...
	protected function save(&$connection)
	{
		...
		switch ($this->scenario) {
			case 'delete':
				$sql = "delete from hr_set_trip where id = :id";
				break;
			case 'new':
				$sql = "insert into hr_set_trip(
						employee_id, audit_user_str, lcu) values (
						:employee_id, :audit_user_str, :lcu)";
				break;
			case 'edit':
				$sql = "update hr_set_trip set 
		...
		if ($this->scenario=='new'){
            ...
            $this->appoint_code="TT".(100000+$this->id);
            Yii::app()->db->createCommand()->update('hr_set_trip', array(
	...
	public function isOccupied($index) {
		...
		$sql = "select a.id from hr_employee_trip a where a.z_index in (10,11,12,13,14) and a.status in (1,2) and a.employee_id='{$employee_id}'";
		$tripRow = Yii::app()->db->createCommand($sql)->queryRow();
		if($tripRow){
		    return true;//不允许删除
        }else{
            return false;
