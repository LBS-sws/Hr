...
class VacationDayForm
{
    ...
    public function setEmployeeList($employee_id){
        $this->employee_id = $employee_id;
        $rows = Yii::app()->db->createCommand()->select("year_day,city,entry_time")->from("hr_employee")
    ...
    public function setVacationId($vacation_id){
        if ($vacation_id === $this->year_type){ //特別處理年假
            $row = Yii::app()->db->createCommand()->select("id")->from("hr_vacation")
    ...
    private function foreachVacationUse($lcd=''){
        ...
        $sum = Yii::app()->db->createCommand()->select("sum(log_time)")->from("hr_employee_leave")
    ...
    //計算在假期規則裡是多少天假期
    private function foreachVacationSum($vacation_id,$bool= true){
        if (in_array($vacation_id,$this->vacation_id_list)||$this->error_bool){
            return false;
        }else{
            $this->vacation_id_list[] = $vacation_id;
        }
        ...
        if($row){
            ...
            if($row["vaca_type"]==$this->year_type&&$yearLeave === "employee"){
                ...
                $this->addRulesNum($row,$bool);//假期規則添加天數
            }
    ...
    private function addRulesNum($row,$bool){
        if($row['log_bool'] == 1){//有假期規則
            ...
            if($bool){ //只調用一次
                ...
                if($row['ass_bool'] == 1){ //有關聯假期規則
                    $ass_id = explode(",",$row['ass_id']);
                    foreach ($ass_id as $id){
                        $this->foreachVacationSum($id,false);
                    }
                }
            }
    ...
    private function addYearLeaveNum($vacation_list){
        switch ($vacation_list["vaca_type"]){
            case $this->year_type://年假（需要添加累計年假的天數）
                ...
                $sum = Yii::app()->db->createCommand()->select("sum(add_num)")->from("hr_staff_year")
